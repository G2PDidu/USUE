...
1. `one = 'Hello'
print(bool(one))`: Выводится строка. 

2. `two = 212
print(float(two))`: Выводится результат с использованием заранее объявленной переменной.

3. `three = None
print(str(three))`: Выводится результат сложиние двух строк в одну.
